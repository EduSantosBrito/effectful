//! Run [`effectful::Effect`] programs inside [Axum](https://docs.rs/axum) handlers on the **same Tokio**
...
//!   `Arc` fields, or a [`effectful::Context`] stack).
//! - This crate **bridges** `State<R>` → `&mut R` for one build step, then runs the effect with
//!   [`effectful_tokio::run_async`] so pending effect steps compose with Tokio I/O.
//!
//! ## Runtime requirements
//!
//! Workspace [`effectful::Effect`] futures are intentionally **not** [`Send`], while Axum route
...
//!
//! **Yes — this crate is built on `effectful_tokio`** (workspace crate `crates/effectful_tokio`).
//! [`crate::routing`] and [`run_with_env`]/[`execute`] all drive effects via
//! [`effectful_tokio::run_async`], so async steps inside `Effect` run on the **same** Tokio runtime as
...
//!
//! ## JSON + [`effectful::schema`]
//!
//! Use [`json::decode_json_schema`] with [`Bytes`](axum::body::Bytes) (or any raw body) to validate
//! JSON via [`Schema::decode_unknown`](effectful::schema::Schema::decode_unknown) (any wire type `I`).
...
mod tests {
  ...
  async fn axum_channel_handler_responds_to_request() {
    ...
    use effectful::{Queue, run_blocking};
