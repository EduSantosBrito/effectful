...
//!
//! fn load_host<A, E, R>() -> ::effect_rs::Effect<A, E, R>
//! where
...
pub type ConfigEnv = ::effect_rs::Context<
  ::effect_rs::Cons<
    ::effect_rs::Service<ConfigProviderKey, ConfigProviderService>,
    ::effect_rs::Nil,
  >,
>;
...
pub fn config_env<P: ConfigProvider + 'static>(provider: P) -> ConfigEnv {
  use ::effect_rs::{Cons, Context, Nil, Service};
  Context::new(Cons(
