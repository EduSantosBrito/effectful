//! Bridge a queue-backed [`effect::channel::QueueChannel`] to Axum: one HTTP exchange is
...
//! ([`QueueChannel`] is used instead of full [`effect::channel::Channel`] so the handle
...
use effect::{Effect, QueueError, box_future, effect};
