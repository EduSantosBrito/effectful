//! Run [`effect::Effect`] programs inside [Axum](https://docs.rs/axum) handlers on the **same Tokio**
...
//!   `Arc` fields, or a [`effect::Context`] stack).
...
//!
//! Workspace [`effect::Effect`] futures are intentionally **not** [`Send`], while Axum route
...
//!
//! **Yes — this crate is built on `effect-tokio`** (workspace crate `crates/effect-rs-tokio`).
...
//! JSON via [`Schema::decode_unknown`](effect::schema::Schema::decode_unknown) (any wire type `I`).
//! Map failures with [`json::JsonSchemaError`] ([`IntoResponse`]) — schema errors become **422** with
//! `path` + `message`.
//!
//! ## Examples
//!
//! See `examples/` (e.g. `cargo run -p effect-axum --example 010_routing_hello`).
