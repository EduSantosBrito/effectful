...
effectful::service_key!(struct ApiTokenKey);
effectful::service_key!(struct MinPriceKey);
...
fn market_report() -> Effect<Report, AppError, Env> {
  effect!(|r: &mut Env| {
    ...
    let filtered: Vec<Quote> = bind
      * Stream::from_effect(fetch_quotes_async())
        .filter(Box::new(move |q: &Quote| q.price >= min_price))
        .run_collect();

    let kept = bind * Ok::<usize, AppError>(filtered.len());

    let total_notional = bind
      * Stream::from_effect(succeed::<Vec<Quote>, AppError, Env>(filtered))
        .map(|q| q.price)
