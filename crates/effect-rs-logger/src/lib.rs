...
use ::effect::{BoxFuture, Effect, EffectHashMap, FiberRef, Get, Here, IntoBind, box_future};
...
effect::service_key!(
  /// Tag for [`EffectLogger`] in an [`effect::Context`] stack.
...
/// Log sink for use as [`effect::Service<EffectLogKey, Self>`](effect::Service); forwards to [`tracing`].
///
/// Extracted from the environment with `~EffectLogger` inside [`effect::effect!`].
...
#[derive(Debug, Clone, ::effect::EffectData)]
...
pub struct LogContext {
  ...
  pub timestamp: ::effect::UtcDateTime,
}

impl LogContext {
  /// Build a context with an explicit UTC timestamp.
  #[inline]
  pub const fn new(timestamp: ::effect::UtcDateTime) -> Self {
    Self { timestamp }
  }
  ...
  pub fn with_now_timestamp() -> Self {
    Self {
      timestamp: ::effect::UtcDateTime::from_std(std::time::SystemTime::now())
...
impl EffectLogger {
  /// Run `inner` with this fiber’s minimum log level overridden to `level` ([`effect::FiberRef::locally`]).
  ...
  pub fn log<R: 'static>(
    ...
    Effect::new(move |_r: &mut R| {
      let emit = MIN_LOG_LEVEL_FIBER_REF.with(|c| match c.borrow().as_ref() {
        None => true,
        Some(fr) => ::effect::run_blocking(fr.get(), ())
      ...
      let annotations = LOG_ANNOTATIONS_FIBER_REF
        .with(|c| {
          c.borrow()
            .as_ref()
            .and_then(|fr| ::effect::run_blocking(fr.get(), ()).ok())
        })
        .unwrap_or_default();
...
/// [`effect::layer_service`] constructor for [`EffectLogger`].
#[inline]
pub fn layer_effect_logger() -> effect::layer::LayerFn<
  impl Fn() -> Result<effect::Service<EffectLogKey, EffectLogger>, Infallible>,
> {
  effect::layer_service(EffectLogger)
}
...
pub fn layer_minimum_log_level(
  initial: LogLevel,
) -> effect::layer::LayerEffect<effect::Service<EffectLogMinLevelKey, FiberRef<LogLevel>>, (), ()> {
  effect::layer::effect(FiberRef::make(move || initial).flat_map(|fr| {
    Effect::new(move |_r: &mut ()| {
      install_min_log_level_fiber_ref(fr.clone());
      Ok(effect::service::<EffectLogMinLevelKey, _>(fr))
    })
  }))
}
...
pub fn layer_composite_logger(
  ...
  effect::layer::effect(Effect::new(move |_r: &mut ()| {
...
/// [`with_log_span`].
#[inline]
pub fn layer_log_metadata() -> effect::layer::LayerEffect<(), (), ()> {
  use ::effect::collections::hash_map;
  ...
  R: Send + 'static,
{
  ...
  Effect::new_async(move |r| {
    ...
    let cur = match ::effect::run_blocking(fr.get(), ()) {
      Ok(m) => m,
      Err(_) => return box_future(async move { inner.run(r).await }),
    };
    let next = ::effect::collections::hash_map::set(&cur, key, value);
    let fr = fr.clone();
  ...
  R: Send + 'static,
{
  ...
  Effect::new_async(move |r| {
    ...
    let cur = ::effect::run_blocking(fr.get(), ()).unwrap_or_default();
...
mod tests {
  ...
  use super::*;
  use ::effect::{Cons, Context, Layer, Nil, Service, run_blocking};
  ...
  mod into_bind_extraction {
    ...
    fn extracts_logger_copy_from_context() {
      let effect: ::effect::Effect<EffectLogger, EffectLoggerError, LogCtx> =
        ::effect::Effect::new_async(move |r| {
