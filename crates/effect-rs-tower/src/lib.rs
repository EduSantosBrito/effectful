//! [`tower::Service`] implementations that run [`effect::Effect`] programs using
//! [`effect_tokio::run_async`], so handlers stay in the Effect interpreter while composing with
//! Tower middleware stacks.
//!
//! This crate depends on **`effect-tokio`** for the async runtime bridge; it does not re-export
...
//! [`effect::SynchronizedRef`] so effectful updates stay serialized when you compose with other
//! `Effect` code.
//!
//! [`EffectService::with_request_metrics`] wraps each call with [`effect::Metric::track_duration`]
//! (typically a [`Metric::timer`](effect::Metric::timer)) and increments an error counter when the
//! handler effect fails.
//!
//! ## Examples
//!
//! See `examples/` (e.g. `cargo run -p effect-tower --example 001_effect_service`) or
...
use effect::{Effect, Metric, QueueError, SynchronizedRef, box_future};
  ...
  F: Fn(&mut S, Req) -> Effect<Res, E, S> + Clone + Send + 'static,
{
  type Response = Res;
  type Error = E;
  /// Not `Send`: [`effect::Effect`] uses non-`Send` [`effect::BoxFuture`]
...
/// Tower [`Service`] shaped like a queue-backed [`effect::channel::QueueChannel`]: each
/// [`Service::call`] runs [`QueueChannel::write`](effect::channel::QueueChannel::write) with the
/// request, then [`QueueChannel::read`](effect::channel::QueueChannel::read) for the response.
...
{
  state: S,
  channel: effect::channel::QueueChannel<Res, Req, S>,
}
  ...
  S: 'static,
{
  ...
  #[inline]
  pub fn new(state: S, channel: effect::channel::QueueChannel<Res, Req, S>) -> Self {
    Self { state, channel }
  }
  ...
  /// Borrow the underlying [`effect::channel::QueueChannel`].
  #[inline]
  pub fn channel(&self) -> &effect::channel::QueueChannel<Res, Req, S> {
    &self.channel
  }
}
...
mod tests {
  use super::*;
  use effect::Metric;
  use effect::channel::QueueChannel;
  use effect::succeed;
  ...
  async fn tower_latency_histogram_records_duration() {
    ...
    assert_eq!(obs.len(), 1);
    assert!(obs[0] > effect::duration::Duration::ZERO);
