...
//! - Optional: [`json_schema`] decodes JSON bodies via [`Schema::decode_unknown`](effect::schema::Schema::decode_unknown)
//!   so [`effect::schema::ParseError`] carries field paths.
//!
//! ## Relation to `effect-tokio`
//!
//! This crate depends only on **`effect`** (and `reqwest`). Async HTTP steps are ordinary
//! [`Effect::new_async`](effect::Effect::new_async) bodies; drive them on Tokio with
//! [`effect::run_async`] or the same symbol re-exported from **`effect-tokio`** as
//! `effect_tokio::run_async` (recommended in Tokio services so sleep/yield stay consistent).
//!
//! ## Example
//!
//! ```ignore
//! use effect::{ctx, run_async, service, Context, Cons, Nil};
//! use effect_reqwest::{ReqwestClientKey, send};
//!
//! type Env = Context<Cons<effect::Service<ReqwestClientKey, reqwest::Client>, Nil>>;
//!
...
//! ## Runnable examples
//!
//! See `examples/` (e.g. `cargo run -p effect-reqwest --example 010_wiremock_get_text`).
...
use effect::data::EffectData;
use effect::schema::{ParseError, Unknown};
...
/// [`effect::Service`] cell holding a [`reqwest::Client`].
pub type ReqwestClientService = effect::Service<ReqwestClientKey, reqwest::Client>;

/// [`effect::Layer`](effect::layer::Layer) that provides a cloneable [`reqwest::Client`].
#[inline]
pub fn layer_reqwest_client(
  client: reqwest::Client,
) -> effect::layer::LayerFn<impl Fn() -> Result<ReqwestClientService, std::convert::Infallible>> {
...
/// [`effect::Service`] cell holding a [`Pool`] of [`PooledClient`].
pub type ReqwestPoolService = effect::Service<ReqwestPoolKey, Pool<PooledClient, Never>>;
...
///
/// The pool is materialized on first [`::effect::layer::Layer::build`] via [`::effect::run_blocking`]
/// inside [`::effect::layer::LayerEffect`], not in this crate’s library surface.
#[inline]
pub fn layer_reqwest_pool(
  capacity: usize,
  ttl: Duration,
) -> ::effect::layer::LayerEffect<ReqwestPoolService, Never, ()> {
  ::effect::layer::effect(
...
fn unknown_from_json_value(value: Value) -> Unknown {
  effect::schema::serde_bridge::unknown_from_serde_json(value)
}
...
mod tests {
  use super::*;
  use effect::schema;
  use effect::{Layer, Scope, run_async, run_blocking, service_env, succeed};
