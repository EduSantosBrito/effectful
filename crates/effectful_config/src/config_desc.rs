...
//! ```rust
//! use std::sync::Arc;
//! use effectful_config::{Config, MapConfigProvider, config_env, ConfigError};
...
impl<T: Send + Sync + 'static> Config<T> {
  ...
  ///
  /// ```rust
  /// use effectful_config::{Config, MapConfigProvider};
  ///
...
impl Config<Duration> {
  /// Load a [`Duration`] using [`effectful::duration::duration::decode`] (Effect.ts `Duration` decode).
  ///
  /// ```rust
  /// use std::time::Duration;
  /// use effectful_config::{Config, MapConfigProvider};
...
///
/// ```rust
/// use effectful_config::{config, Config, MapConfigProvider};
///
...
mod tests {
  ...
  fn config_run_via_effect() {
    ...
    let env = config_env(p);
    let host: String =
      effectful::run_blocking(Config::string("HOST").run::<String, ConfigError, _>(), env).unwrap();
    assert_eq!(host, "localhost");
  }
  ...
  fn config_run_error_propagated() {
    ...
    let p = MapConfigProvider::from_pairs::<[(&str, &str); 0], _, _>([]);
    let env = config_env(p);
    let result: Result<String, ConfigError> = effectful::run_blocking(
  ...
  fn load_current_with_ambient_provider() {
    ...
    let eff = with_config_provider(
      effectful::Effect::new(|_| Config::string("X").load_current()),
      p,
    );
    assert_eq!(effectful::run_blocking(eff, ()).unwrap(), "from-ambient");
  }
