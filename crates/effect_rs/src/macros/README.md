...
Rust cannot mix `macro_rules!` and `#[proc_macro]` in one crate, so the `effect` crate **re-exports** them at the root (`pub use effect_rs_macro::…`, `pub use effect_rs_proc_macro::…`) and exposes `macros::effect_rs::effect` as a stable path for the procedural `effect!` macro.

## What lives here

| Item | Role |
|------|------|
| `macros::effect` | Re-export of `effect_rs_proc_macro::effect` — do-notation for `Effect`. |
| Re-exports (see [`mod.rs`](mod.rs)) | Same symbols as `use effect_rs::{pipe, ctx, …}` for module-qualified imports. |
...
- Crates `crates/effect_rs_macro`, `crates/effect_rs_proc_macro` — implementations.
