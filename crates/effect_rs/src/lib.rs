...
// `clone` on `Copy` `Result`, etc.); keep `cargo clippy -p effect_rs --all-targets -- -D warnings` green.
#![cfg_attr(
  test,
  allow(
...
// Lets `::effect_rs::…` in `effect_rs_macro` expansions resolve when those macros are used inside this crate.
extern crate self as effect_rs;

pub use effect_rs_macro::{ctx, err, layer_graph, layer_node, pipe, req, service_def, service_key};
pub use effect_rs_proc_macro::{EffectData, effect, effect_tagged};
...
// External crates that use `effect_rs::channel::Channel` etc. keep working.
...
// Callers can use `effect_rs::im::Vector`, `effect_rs::im::OrdSet`, etc. without
// adding a separate `im` dependency, and they're guaranteed version-compatible
// with all `effect_rs::collections` types (which are type aliases of `im` types).
pub use im;
