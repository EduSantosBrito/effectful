...
This appendix is a practical guide for converting existing async Rust code to effect_rs. It covers the most common patterns and their effect_rs equivalents, with migration steps for each.
...
In effect_rs, functions *describe* things:
...
1. Start with leaf functions (those with no effect_rs dependencies yet) — convert them first.
