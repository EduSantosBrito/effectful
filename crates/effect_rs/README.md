...
Single-page index of modules and primary APIs under `crates/effect_rs/src/`. Each line states **when** to reach for it and **how** it fits the system (lazy `Effect` graphs, compile-time `R`, boundaries at `run_*`). For stratum notes and design detail, see `src/**/README.md` where present.
...
|-----------|-------------------|
| **Public re-exports** | Prefer `use effect_rs::{Effect, …}` for the supported surface; module paths (`effect::kernel::…`) remain for precision and older call sites. |
| **`extern crate self as effect`** | Lets proc-macros emit `::effect_rs::…` paths reliably when expanding inside this crate. |
