//! Compare **bind-free** [`effect!`](::effect_rs::effect) expansion vs hand-written [`Effect::new`](::effect_rs::Effect::new).
//!
//! After the proc-macro fast path (no `~` → `Effect::new`), both should compile to very similar
//! code and show comparable timings. Run:
//!
//! ```bash
//! cargo bench -p effect_rs --bench effect_macro_bind_free
//! ```

use ::effect_rs::{Effect, effect};
