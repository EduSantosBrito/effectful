//! [`tower::Service`] implementations that run [`effectful::Effect`] programs using
//! [`effectful_tokio::run_async`], so handlers stay in the Effect interpreter while composing with
//! Tower middleware stacks.
//!
//! This crate depends on **`effectful_tokio`** for the async runtime bridge; it does not re-export
//! [`effectful_tokio::TokioRuntime`].
//!
//! ## Concurrency
//!
...
//! [`effectful::SynchronizedRef`] so effectful updates stay serialized when you compose with other
//! `Effect` code.
//!
//! [`EffectService::with_request_metrics`] wraps each call with [`effectful::Metric::track_duration`]
  ...
  F: Fn(&mut S, Req) -> Effect<Res, E, S> + Clone + Send + 'static,
{
  type Response = Res;
  type Error = E;
  /// Not `Send`: [`effectful::Effect`] uses non-`Send` [`effectful::BoxFuture`]
...
/// Tower [`Service`] shaped like a queue-backed [`effectful::channel::QueueChannel`]: each
/// [`Service::call`] runs [`QueueChannel::write`](effectful::channel::QueueChannel::write) with the
/// request, then [`QueueChannel::read`](effectful::channel::QueueChannel::read) for the response.
...
{
  state: S,
  channel: effectful::channel::QueueChannel<Res, Req, S>,
}
  ...
  S: 'static,
{
  ...
  #[inline]
  pub fn new(state: S, channel: effectful::channel::QueueChannel<Res, Req, S>) -> Self {
    Self { state, channel }
  }
  ...
  /// Borrow the underlying [`effectful::channel::QueueChannel`].
  #[inline]
  pub fn channel(&self) -> &effectful::channel::QueueChannel<Res, Req, S> {
    &self.channel
  }
}
  ...
  async fn tower_latency_histogram_records_duration() {
    ...
    let mut svc = EffectService::new((), |_env: &mut (), _x: u32| {
      Effect::new_async(move |_r: &mut ()| {
        effectful::box_future(async move {
    ...
    assert_eq!(obs.len(), 1);
    assert!(obs[0] > effectful::duration::Duration::ZERO);
