...
//! - Optional: [`json_schema`] decodes JSON bodies via [`Schema::decode_unknown`](effect_rs::schema::Schema::decode_unknown)
//!   so [`effect_rs::schema::ParseError`] carries field paths.
//!
//! ## Relation to `effect_rs_tokio`
//!
//! This crate depends only on **`effect`** (and `reqwest`). Async HTTP steps are ordinary
//! [`Effect::new_async`](effect_rs::Effect::new_async) bodies; drive them on Tokio with
//! [`effect_rs::run_async`] or the same symbol re-exported from **`effect_rs_tokio`** as
//! `effect_tokio::run_async` (recommended in Tokio services so sleep/yield stay consistent).
//!
//! ## Example
//!
//! ```ignore
//! use effect_rs::{ctx, run_async, service, Context, Cons, Nil};
//! use effect_reqwest::{ReqwestClientKey, send};
//!
//! type Env = Context<Cons<effect_rs::Service<ReqwestClientKey, reqwest::Client>, Nil>>;
//!
...
//! ## Runnable examples
//!
//! See `examples/` (e.g. `cargo run -p effect_rs_reqwest --example 010_wiremock_get_text`).
...
use effect_rs::data::EffectData;
use effect_rs::schema::{ParseError, Unknown};
...
/// [`effect_rs::Service`] cell holding a [`reqwest::Client`].
pub type ReqwestClientService = effect_rs::Service<ReqwestClientKey, reqwest::Client>;

/// [`effect_rs::Layer`](effect_rs::layer::Layer) that provides a cloneable [`reqwest::Client`].
#[inline]
pub fn layer_reqwest_client(
  client: reqwest::Client,
) -> effect_rs::layer::LayerFn<impl Fn() -> Result<ReqwestClientService, std::convert::Infallible>>
{
...
/// [`effect_rs::Service`] cell holding a [`Pool`] of [`PooledClient`].
pub type ReqwestPoolService = effect_rs::Service<ReqwestPoolKey, Pool<PooledClient, Never>>;
...
///
/// The pool is materialized on first [`::effect_rs::layer::Layer::build`] via [`::effect_rs::run_blocking`]
/// inside [`::effect_rs::layer::LayerEffect`], not in this crate’s library surface.
#[inline]
pub fn layer_reqwest_pool(
  capacity: usize,
  ttl: Duration,
) -> ::effect_rs::layer::LayerEffect<ReqwestPoolService, Never, ()> {
  ::effect_rs::layer::effect(
...
fn unknown_from_json_value(value: Value) -> Unknown {
  effect_rs::schema::serde_bridge::unknown_from_serde_json(value)
}
...
mod tests {
  ...
  use effect_rs::{Layer, Scope, run_async, run_blocking, service_env, succeed};
