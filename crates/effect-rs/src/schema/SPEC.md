...
- **Data:** Keep **`EffectData`** (derive via `effect_proc_macro::EffectData`) as the **standard “dataclass” story** (structural `PartialEq` / `Eq` / `Hash`); **integrate** schemas so users do not maintain two unrelated models.
...
- **`Unknown`:** Has **`Null`**, **`Bool`**, **`I64`**, **`F64`**, **`String`**, **`Array`**, **`Object`**. JSON bridges (`effect-axum`, `effect-reqwest`) map non-integer **`serde_json::Number`** values to **`F64`**.
...
- **Examples** in `crates/effect-rs/examples/` reference new combinators with **runnable** code.

**Delivery (`@TESTING.md`):** **Doc examples** are **tested** if the workspace uses **doctest**—otherwise **example binaries** serve as the contract (verify with `cargo test -p effect-rs` / CI).
...
**Delivery (`@TESTING.md`):** Reviewers use this table as a **checklist** for every PR touching `crates/effect-rs/src/schema/`.
