...
// `clone` on `Copy` `Result`, etc.); keep `cargo clippy -p effect --all-targets -- -D warnings` green.
#![cfg_attr(
  test,
  allow(
...
// Lets `::effect::…` in `effect-macro` expansions resolve when those macros are used inside this crate.
extern crate self as effect;

pub use effect_macro::{ctx, err, layer_graph, layer_node, pipe, req, service_def, service_key};
pub use effect_proc_macro::{EffectData, effect, effect_tagged};
...
// External crates that use `effect::channel::Channel` etc. keep working.
...
// Callers can use `effect::im::Vector`, `effect::im::OrdSet`, etc. without
// adding a separate `im` dependency, and they're guaranteed version-compatible
// with all `effect::collections` types (which are type aliases of `im` types).
pub use im;
