...
This appendix is a practical guide for converting existing async Rust code to effect-rs. It covers the most common patterns and their effect-rs equivalents, with migration steps for each.
...
In effect-rs, functions *describe* things:
...
1. Start with leaf functions (those with no effect-rs dependencies yet) — convert them first.
