...
When a user submits a form with five invalid fields, they deserve to know about all five — not just the first one you found. `ParseErrors` is effect-rs's solution: errors accumulate across an entire parse, and you report them all at once.

## ParseError vs ParseErrors

```rust
use effect::schema::{ParseError, ParseErrors};
