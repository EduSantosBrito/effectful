...
`serde_json::Value` is an excellent data type, but it's stringly typed: `value["name"]` gives you an `Option<&Value>` and there's no structure around parse errors, path tracking, or accumulation. `Unknown` wraps the same idea but integrates with effect-rs's schema parser, which gives you:
...
```rust
use effect::schema::serde_bridge::unknown_from_serde_json;
