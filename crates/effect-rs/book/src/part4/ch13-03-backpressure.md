...
Your options are: block the producer, drop events, or buffer them. Each is correct in different contexts. effect-rs makes the choice explicit via `BackpressurePolicy`.

## BackpressurePolicy

```rust
use effect::BackpressurePolicy;
...
```rust
use effect::{stream_from_channel_with_policy, BackpressurePolicy};
use std::sync::mpsc;
