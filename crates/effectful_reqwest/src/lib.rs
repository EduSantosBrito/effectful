...
//! - Optional: [`json_schema`] decodes JSON bodies via [`Schema::decode_unknown`](effectful::schema::Schema::decode_unknown)
//!   so [`effectful::schema::ParseError`] carries field paths.
//!
//! ## Relation to `effectful_tokio`
//!
//! This crate depends only on **`effect`** (and `reqwest`). Async HTTP steps are ordinary
//! [`Effect::new_async`](effectful::Effect::new_async) bodies; drive them on Tokio with
...
use effectful::data::EffectData;
use effectful::schema::{ParseError, Unknown};
...
pub type ReqwestClientService = effectful::Service<ReqwestClientKey, reqwest::Client>;

/// [`effectful::Layer`](effectful::layer::Layer) that provides a cloneable [`reqwest::Client`].
#[inline]
pub fn layer_reqwest_client(
  client: reqwest::Client,
) -> effectful::layer::LayerFn<impl Fn() -> Result<ReqwestClientService, std::convert::Infallible>>
{
...
pub type ReqwestPoolService = effectful::Service<ReqwestPoolKey, Pool<PooledClient, Never>>;
...
///
/// The pool is materialized on first [`::effectful::layer::Layer::build`] via [`::effectful::run_blocking`]
/// inside [`::effectful::layer::LayerEffect`], not in this crate’s library surface.
#[inline]
pub fn layer_reqwest_pool(
  capacity: usize,
  ttl: Duration,
) -> ::effectful::layer::LayerEffect<ReqwestPoolService, Never, ()> {
  ::effectful::layer::effect(
...
fn unknown_from_json_value(value: Value) -> Unknown {
  effectful::schema::serde_bridge::unknown_from_serde_json(value)
}
...
mod tests {
  ...
  async fn get_helper_fetches_text() {
    ...
    let body = run_async(
      get::<Response, Error, _>(url).flat_map(|resp: Response| {
        effectful::from_async(move |_r: &mut _| async move { resp.text().await })
      }),
      env,
    )
    .await
  ...
  fn json_schema_error_schema_debug() {
    let e = JsonSchemaError::Schema(effectful::schema::ParseError::new("field", "invalid"));
    let _ = format!("{e:?}");
  }
