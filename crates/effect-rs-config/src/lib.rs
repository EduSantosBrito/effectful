...
//!
//! fn load_host<A, E, R>() -> ::effect::Effect<A, E, R>
//! where
...
pub type ConfigEnv = ::effect::Context<
  ::effect::Cons<::effect::Service<ConfigProviderKey, ConfigProviderService>, ::effect::Nil>,
>;
...
pub fn config_env<P: ConfigProvider + 'static>(provider: P) -> ConfigEnv {
  use ::effect::{Cons, Context, Nil, Service};
  Context::new(Cons(
