...
// External crates that use `effectful::channel::Channel` etc. keep working.
...
// Callers can use `effectful::im::Vector`, `effectful::im::OrdSet`, etc. without
// adding a separate `im` dependency, and they're guaranteed version-compatible
// with all `effectful::collections` types (which are type aliases of `im` types).
pub use im;
