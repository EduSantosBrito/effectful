//! Run [`effect_rs::Effect`] programs inside [Axum](https://docs.rs/axum) handlers on the **same Tokio**
...
//!   `Arc` fields, or a [`effect_rs::Context`] stack).
...
//!
//! Workspace [`effect_rs::Effect`] futures are intentionally **not** [`Send`], while Axum route
...
//!
//! **Yes — this crate is built on `effect_rs_tokio`** (workspace crate `crates/effect_rs_tokio`).
...
//!
//! ## JSON + [`effect_rs::schema`]
//!
//! Use [`json::decode_json_schema`] with [`Bytes`](axum::body::Bytes) (or any raw body) to validate
//! JSON via [`Schema::decode_unknown`](effect_rs::schema::Schema::decode_unknown) (any wire type `I`).
//! Map failures with [`json::JsonSchemaError`] ([`IntoResponse`]) — schema errors become **422** with
//! `path` + `message`.
//!
//! ## Examples
//!
//! See `examples/` (e.g. `cargo run -p effect_rs_axum --example 010_routing_hello`).
...
mod tests {
  ...
  async fn axum_channel_handler_responds_to_request() {
    ...
    use effect_rs::{Queue, run_blocking};
