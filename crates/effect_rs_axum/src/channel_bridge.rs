//! Bridge a queue-backed [`effect_rs::channel::QueueChannel`] to Axum: one HTTP exchange is
...
//! ([`QueueChannel`] is used instead of full [`effect_rs::channel::Channel`] so the handle
...
use effect_rs::{Effect, QueueError, box_future, effect};
